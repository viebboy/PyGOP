...
class HeMLGOP(_Model):
    """Heterogeneous Multilayer Generalized Operational Perceptron (HeMLGOP) model

    This class implements the HeMLGOP algorithm to learn a heterogeneous multilayer
    ...
    Examples:
        ...
        >>> model = models.HeMLGOP() # create a model instance
        ...
        # save model to 'hemlgop.pickle'
        >>> model.save('hemlgop.pickle')

        # load model from 'hemlgop.pickle' and finetune with another data and
        # (possibly different) parameter settings
        >>> model.load('hemlgop.pickle')
        ...
        if not train_states['is_finished']:
            for layer_iter in range(train_states['layer_iter'], params['max_layer']):
                ...
                for block_iter in range(train_states['block_iter'], params['max_block']):
                    ...
                    # perform operator set evaluation
